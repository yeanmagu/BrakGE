...
namespace BrakGeWeb
{
    public partial class Facturacion : PaginaBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    Session["Titulo"] = "Gestion De  Documentos";
                    FillDocumentos();
        ...
        private void FillDocumentos()
        {
            try
            {
                Session["ListDocumentos"] = BllDocumentos.ToListBySw(3);
            ...
            try
            {
                if (e.CommandName != "Page")
                {
                    ...
                    if (Rows.Exists(b => b.Id.ToString() == e.CommandArgument.ToString()))
                    {
                        ...
                        //NroOrdenCompra.Text=Row.NroOrdenDeCompra;
                        //NroProveedor.Text=Row.NroProveedor;
                        //MIGO.Text=Row.MIGO;
                        NumeroCotizacion.Text=Row.NumeroCotizacion.ToString();
                        fillDeta(Deta);
                        TotalGuardar.Text = Row.Total.ToString();
                        TotalSubGuardar.Text = Row.TotalSub.ToString();
                        TotalIvaGuardar.Text = Row.TotalIva.ToString();
                        Total.Text = String.Format("{0:C2}", Row.Total.ToString());
                        TotalSub.Text = String.Format("{0:C2}", Row.TotalSub.ToString());
                        TotalIva.Text = String.Format("{0:C2}", Row.TotalIva.ToString());
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    if ((Documento.Text) != "")
                    {
                        ...
                        var CO=ObjGrabar.GetCotizacion(ObjGrabar.IdTipoMovimiento);
                        ObjGrabar.NumeroCotizacion=CO;
                        
                        //ObjGrabar.NroOrdenDeCompra = NroOrdenCompra.Text;
                        //ObjGrabar.NroProveedor = NroProveedor.Text;
                        //ObjGrabar.MIGO = MIGO.Text;
                        //ObjGrabar.NroCotizacion = NroCotizacion.Text;
                        var tipoMov = BllTipoMovimiento.GetById(ObjGrabar.IdTipoMovimiento);

                        var FP = BllFormaDePago .GetById(ObjGrabar.IdFormaPago);
                        if (FP.Credito==true)
                        {
                            ObjGrabar.EstadoPago="PP";
                        }
                        else
                        {
                        ...
                        if (r > 0)
                        {
                            NumeroCotizacion.Text=ObjGrabar.NumeroCotizacion.ToString();
                            ...
                            foreach (var item in ListDeta)
                            {
                                ...
                                if (it.Id > 0)
                                {
                                    ...
                                    {
                                        if (item.Cantidad <= it.CantidadDisponible)
                                        {
                                            it.CantidadDisponible -= item.Cantidad;
                                        }
                                        else
                                ...
                                Log.EscribirTraza("Inventario de Producto " + Prod.Descripcion + " Cantidad Existente actual :" + Prod.CantidadExistente.ToString() + "  del Documento  " + ObjGrabar.TipoMovimiento + " Nro." + ObjGrabar.Id.ToString());
                            }
                    ...
                    {
                        Msj1.Text = "Por Favor Agregue un cliente";
                        Type1.Text = "error";
                        Documento.Focus();
                    }
                     scope.Complete();
        ...
        protected void BtnAnular_Command(object sender, CommandEventArgs e)
        {
            try
            {
                if (e.CommandName != "Page")
                {
                    ...
                    {
                        Row = Rows.Where(b => b.Id.ToString() == e.CommandArgument.ToString()).First();
                        Response.Redirect("NotasDeDevolucion.aspx?Id=" +Row.Id.ToString(),false);
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void Bodega_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    TipoMovimiento.DataSource = BllTipoMovimiento.ToListBySw(3,int.Parse(Bodega.SelectedValue));
                    TipoMovimiento.DataTextField = "Descripcion";
                    TipoMovimiento.DataValueField = "Id";
                    TipoMovimiento.DataBind();
        ...
        protected void Imprimir_Command(object sender, CommandEventArgs e)
        {
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "Print('"+e.CommandArgument.ToString()+"');", true);
        }

        protected void Imprimir_Click(object sender, EventArgs e)
        {
            var id = TxtId.Text;
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "Print('" + id + "');", true);
        }
        ...
        protected void CrearReclamo_Command(object sender, CommandEventArgs e)
        {
            try
            {
                if (e.CommandName != "Page")
                {
                    ...
                    if (Rows.Exists(b => b.Id.ToString() == e.CommandArgument.ToString()))
                    {
                        ...
                        {
                            Response.Redirect("Reclamos.aspx?Id=" + Row.Id.ToString(), false);
                        }
                        else
                        {
                            Msj1.Text = "No se puede generar reclamacion al documento.";
                            Type1.Text = "warning";
                        }
