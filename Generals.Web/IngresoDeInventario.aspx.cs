...
namespace BrakGeWeb
{
    public partial class IngresoDeInventario : PaginaBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    Session["Titulo"] = "Ingreso de Inventario";
                    FillDocumentos();
        ...
        private void GuardarTipo()
        {
            try
            {
                if ((Documento.Text) != "")
                {
                    ...
                    ObjGrabar.Total=decimal.Parse(Total.Text);
                    ObjGrabar.Total_Descuento=decimal.Parse(TotalDescuentos.Text);
                    ObjGrabar.TotalSub=decimal.Parse(TotalSub.Text);
                    ...
                    if (r > 0)
                    {
                        ...
                        foreach (var item in ListDeta)
                        {
                            ...
                            it = it.GetById(Prod.Id,int.Parse(Bodega.SelectedValue));
                            if (it.Id>0)
                            {
                                if (tipoMov.IdSw == 2)
                                {
                            ...
                            {
                                it.IdItem=Prod.Id;
                                it.IdBodega= int.Parse(Bodega.SelectedValue);
        ...
        protected void Modificar()
        {
            try
            {
                ...
                {
                    var obj = BllDocumentos.GetById(int.Parse(TxtId.Text));
                    //obj.Descripcion = TxtNombre.Text;
                    //obj.Estado = Estado.Checked;

                    int r = BllDocumentos.Update(obj);
                    if (r > 0)
                    {
                        FillDocumentos();
                        TxtId.Text = r.ToString();
        ...
        protected void Detalle_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    _Total += (Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "PrecioCompra")) * Convert.ToInt16(DataBinder.Eval(e.Row.DataItem, "Cantidad")));
                    cant += Convert.ToInt16(DataBinder.Eval(e.Row.DataItem, "Cantidad"));
                }
                else if (e.Row.RowType == DataControlRowType.Footer)
                {
                    e.Row.Cells[1].Text = "TOTAL:";
                    e.Row.Cells[2].Text = cant.ToString();
                    e.Row.Cells[3].Text = String.Format("{0:C2}", (Math.Round(float.Parse(_Total.ToString()), 2)));// _Total.ToString();
                    e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                    e.Row.Font.Bold = true;
                    if (e.Row.RowType == DataControlRowType.Footer)
                    {
                        e.Row.Cells[3].Text = String.Format("{0:C2}", (Math.Round(float.Parse(_Total.ToString()), 2))); ;
                        
                    }
                    Total.Text = String.Format("{0:C2}", (Math.Round(float.Parse(_Total.ToString()), 2)));
                    TotalSub.Text = String.Format("{0:C2}", (Math.Round(float.Parse(_Total.ToString()), 2)));
                }
            }
            catch (Exception err)
            {
        ...
        protected void Detalle_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
           ...
           TextBox Prec = (TextBox)Detalle.Rows[e.RowIndex].FindControl("TxtPrecioCompra");
           GridViewRow row = (GridViewRow)c.NamingContainer;          
           UpdateItem(int.Parse(id.Text),int.Parse(c.Text),decimal.Parse(Pre.Text),decimal.Parse(Prec.Text));
           Detalle.EditIndex = -1;
           Detalle.DataSource = (List<BllDetalleDocumento>)Session["ListDeta"];
           Detalle.DataBind();
        }
       
        protected void UpdateItem(int idProd, int canti,decimal Pre, decimal prec)
        {
            try
            {
                var Lis = (List<BllDetalleDocumento>)Session["ListDeta"];
