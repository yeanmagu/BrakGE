...
namespace BrakGeWeb
{
    public partial class Cotizacion : PaginaBase
    {
        ...
        private void FillDocumentos()
        {
            try
            {
                Session["ListDocumentos"] = BllDocumentos.ToListBySw(1);
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    if ((IdDocumentoAnterior.Text) != "")
                    {
                        ...
                        ObjGrabar.IdFormaPago = int.Parse(FormaDePago.SelectedValue);
                        var tipoMov = BllTipoMovimiento.GetByIdSw(SW, ObjGrabar.IdBodega);
                        ...
                        if (r > 0)
                        {
                            ...
                            DocAnt.IdEstadoDocumento = 1003;
                            BllDocumentos.Update(DocAnt);
                            //var tipoMov = BllTipoMovimiento.GetById(2002);
                            foreach (var item in ListDeta)
                            {
                                if (item.Cantidad > 0)
                                {
        ...
        protected void GridDocumentos_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (e.Row.Cells[4].Text == "Facturado")
                    {
                        Btn.Visible=false;
                        Btn.Enabled=false;
        ...
        protected void Ot_Command(object sender, CommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "OrdenDeTrabajo")
                {
                    BllDocumentos Row = new BllDocumentos();
                    ...
                    if (Rows.Exists(b => b.Id.ToString() == e.CommandArgument.ToString()))
                    {
                        ...
                        {
                            Msj1.Text = "Documento Esta " +Row.EstadoDocumento;
                            Type1.Text = "warning";
                        }
        ...
        protected void BtnOt_Click(object sender, EventArgs e)
        {
            ...
            {
                Msj1.Text = "NO SE HA SELECCIONADO NINGUNA COTIZACION PARA CREAR UNA OT...!";
                Type1.Text = "error";
                return;
            }
        }
