...
namespace BrakGeWeb
{
    public partial class TipoActa : PaginaBase
    {
        ...
        private void FillTipoActa()
        {
            try
            {
                Session["ListTipoActa"] = BllTipoActa.ToList();
                if (!string.IsNullOrEmpty(Session["ListTipoActa"].ToString()))
                {
                    GridTipoActa.DataSource = (List<BllTipoActa>)Session["ListTipoActa"];
                    GridTipoActa.DataBind();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void BtnSelect_Command(object sender, CommandEventArgs e)
        {
            try
            {
                ...
                {
                    BllTipoActa Row = new BllTipoActa();

                    List<BllTipoActa> Rows = new List<BllTipoActa>();

                    Rows = (List<BllTipoActa>)Session["ListTipoActa"];
        ...
        protected void btneliminarGridView_Command(object sender, CommandEventArgs e)
        {
            try
            {
                if (e.CommandName != "Page")
                {
                    bool r = BllTipoActa.Delete(int.Parse(e.CommandArgument.ToString()));
                    if (r == true)
                    {
                        FillTipoActa();
        ...
        private void GuardarTipo()
        {
            try
            {
                if (BllTipoActa.ExisteDescri(TxtNombre.Text) == false)
                {
                    BllTipoActa ObjGrabar = new BllTipoActa();
                    
                    ObjGrabar.Descripcion = TxtNombre.Text;
                    ObjGrabar.Estado = ChkEstado.Checked;

                    int r = BllTipoActa.Add(ObjGrabar);
                    if (r > 0)
                    {
                        FillTipoActa();
        ...
        protected void Modificar()
        {
            try
            {
                if (BllTipoActa.GetById(int.Parse(TxtId.Text)).Id>0)
                {
                    var obj = BllTipoActa.GetById(int.Parse(TxtId.Text));
                    obj.Descripcion = TxtNombre.Text;
                    obj.Estado = ChkEstado.Checked;

                    int r = BllTipoActa.Update(obj);
                    if (r > 0)
                    {
                        FillTipoActa();
                        TxtId.Text = r.ToString();
        ...
        protected void TxtBusqueda_TextChanged(object sender, EventArgs e)
        {
            try
            {
                Session["ListTipoActa"] = BllTipoActa.ToList(TxtBusqueda.Text.Trim());
                if (!string.IsNullOrEmpty(Session["ListTipoActa"].ToString()))
                {
                    GridTipoActa.DataSource = (List<BllTipoActa>)Session["ListTipoActa"];
                    GridTipoActa.DataBind();
        ...
        protected void GridTipoActa_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                GridTipoActa.PageIndex = e.NewPageIndex;
                GridTipoActa.DataSource = (List<BllTipoActa>)Session["ListTipoActa"];
                GridTipoActa.DataBind();
            }
