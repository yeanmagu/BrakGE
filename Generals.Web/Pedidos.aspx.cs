...
namespace BrakGeWeb
{
    public partial class Pedidos : PaginaBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    Session["Titulo"] = "ORDENES DE COMPRA";
                    FillDocumentos();
        ...
        private void FillVendedores()
        {
            try
            {
                ...
                Vendedor.DataBind();
                Vendedor.Items.Insert(0, new ListItem("Seleccione Facturador", "0"));
        ...
        private void FillDocumentos()
        {
            try
            {
                Session["ListDocumentos"] = BllDocumentos.ToListBySw(4);
            ...
            try
            {
                if (e.CommandName == "Aprobar")
                {
                    ...
                    if (Rows.Exists(b => b.Id.ToString() == e.CommandArgument.ToString()))
                    {
                        ...
                        if (Row.EstadoDocumento!="Aprobado")
                        {
        ...
        protected void btnPopUp_Click(object sender, EventArgs e)
        {
            ...
            NuevoProv.Visible = true;
            BuscarProv.Visible = false;
        }
        ...
        private void FillItem(int idProv)
        {
            try
            {
                Session["ListItem"] = BllItem.ToListByProv(idProv);
        ...
        protected void ItemBusqueda_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                Session["ListItem"] = BllItem.ToList(ItemBusqueda.Text,int.Parse(IdCliente.Text));
                if (!string.IsNullOrEmpty(Session["ListItem"].ToString()))
                {
                    GridItem.DataSource = (List<BllItem>)Session["ListItem"];
                    GridItem.DataBind();
            ...
            {
                FillItem(int.Parse(IdCliente.Text));
            }
        ...
        protected void Detalle_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                else if (e.Row.RowType == DataControlRowType.Footer)
                {
                    ...
                    e.Row.Cells[6].Text = String.Format("{0:C2}",_Total);
                    e.Row.Cells[4].Text = String.Format("{0:C2}", _Iva);
        ...
        protected void Detalle_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
           ...
           //{
               ...
               //Detalle.DataSource = (List<BllDetalleDocumento>)Session["ListDeta"];
               //Detalle.DataBind();
            fillDeta((List<BllDetalleDocumento>)Session["ListDeta"]);
           //}
           //else
           //{
        ...
        protected void Bodega_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    TipoMovimiento.DataSource = BllTipoMovimiento.ToListBySw(4,int.Parse(Bodega.SelectedValue));
                    TipoMovimiento.DataTextField = "Descripcion";
                    TipoMovimiento.DataValueField = "Id";
                    TipoMovimiento.DataBind();
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    if ((IdDocumentoAnterior.Text) != "")
                    {
                        ...
                        ObjGrabar.IdEstadoDocumento = 1;
                        ObjGrabar.FechaCreacion = DateTime.Parse(Fecha.Text);
                        ObjGrabar.IdDocumentoAnterior=ObjGrabar.Id;
                       
                        ObjGrabar.IdFormaPago = int.Parse(FormaDePago.SelectedValue);
                        var tipoMov = BllTipoMovimiento.GetByIdSw(2, ObjGrabar.IdBodega);
                        ObjGrabar.IdTipoMovimiento = tipoMov.Id;
                        ObjGrabar.IdUsuario = int.Parse(Usuario.id_usuario.ToString());
                        ObjGrabar.Total = decimal.Parse(TotalGuardar.Text);
                        ObjGrabar.Total_Descuento = decimal.Parse(TotalDescuentosGuardar.Text);
                        ObjGrabar.TotalSub = decimal.Parse(TotalSubGuardar.Text);
                        ...
                        if (r > 0)
                        {
                            ...
                            TxtId.Text = r.ToString();
                            var DocAnt= BllDocumentos.GetById(ObjGrabar.IdDocumentoAnterior);
                            DocAnt.IdEstadoDocumento = 1002;
                            var cli = BllPersonas.GetById(DocAnt.IdCliente);
                            var diasCre=cli.DiasCreditoProv.Value;
                            if (diasCre== null)
                            {
                                diasCre=0;
                            }
                            DocAnt.FechaVencimiento = DateTime.Now.AddDays(diasCre);
                            BllDocumentos.Update(DocAnt);
                            foreach (var item in ListDeta)
                            {
                                item.IdDocumento = r;
                                var Prod = BllItem.GetById(item.IdProducto);
                                BllDetalleDocumento.Add(item);
                                //var tipoMov = BllTipoMovimiento.GetByIdSw(3, ObjGrabar.IdBodega);
                                var it = new BllInventario();
                                it = it.GetById(Prod.Id, (ObjGrabar.IdBodega));
                                if (it.Id > 0)
                                {
        ...
        protected void BtnAprobar_Click(object sender, EventArgs e)
        {
            ...
            {
                Msj1.Text = "NO SE HA SELECCIONADO NINGUN PEDIDO PARA APROBAR...!";
                Type1.Text = "error";
                return;
            }
        }
        ...
        protected void Documento_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (Documento.Text != "")
                {
                    //var Cli = BllPersonas.GetProvByDocument(Documento.Text);
                    //if (Cli.Id > 0)
                    //{
                    //    BuscarCliente(Cli);
                    //}
        ...
        private void FillProvedor()
        {
            try
            {
                Session["ListProv"] = BllPersonas.ToListProveedor();
        ...
        protected void BuscarProvedor_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (BuscarProvedor.Text != "")
                {
                    ...
                    Session["ListProv"] = BllPersonas.ToListProveedor(BuscarProvedor.Text);
                    GridProvedor.DataSource = (List<BllPersonas>)Session["ListProv"];
                    GridProvedor.DataBind();
                }
                else
