...
namespace BrakGeWeb
{
    public partial class GenerarPagoFacturasAntiguas : PaginaBase
    {
        List<BllCuotasPagoPendiente> DetallePago= new List<BllCuotasPagoPendiente>();
        BllFormaPagoPendientes Forma=new BllFormaPagoPendientes();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //ValidarAutorizacion();
                    Session["Titulo"] = "Generar Acuerdos De Facturas";
                    FillDocumentos();
        ...
        protected void GenerarAcuerdoPagoCuentasPendientess(decimal deuda,int nroCuotas,int idPago ,int Modo)
        {
            try
            {
        ...
        private void GuardarTipo()
        {
            try
            {
                ...
                {
                    var Obj = new BllAcuerdoPagoCuentasPendientes();

                    Obj.IdFactura = int.Parse(IdFactura.Text);
                    Obj.NroCuotas = int.Parse(Nrocuotas.Text);
                    Obj.FechaCreacion = DateTime.Parse(FechaAcuerdo.Text);
                    Obj.Estado = true;
                    Obj.DiasPago = int.Parse(ModoPago.SelectedValue);
                    Obj.FormaPago = ModoPago.SelectedItem.ToString();
                    ...
                    if (Id.Text == "")
                    {
                        ...
                        if (r > 0)
                        {
                            ...
                            var DP = new BllCuotasPagoPendiente();
                            DetallePago = (List<BllCuotasPagoPendiente>)Session["ListDeta"];
        ...
        protected void BtnGenerarPlan_Click(object sender, EventArgs e)
        {
            try
            {
                GenerarAcuerdoPagoCuentasPendientess(decimal.Parse(SaldoTotalGuardar.Text), int.Parse(Nrocuotas.Text), int.Parse(IdFactura.Text), int.Parse(ModoPago.SelectedValue));
        ...
        protected void FillAcuerdos(int NroFactura)
        {
            try
            {
                ...
                {
                    GridAcuerdos.DataSource=(List<BllAcuerdoPagoCuentasPendientes>)Session["ListAcuerdos"];
        ...
        protected void EditarAcuerdo_Command(object sender, CommandEventArgs e)
        {
            try
            {
                ...
                {
                    BllAcuerdoPagoCuentasPendientes Row = new BllAcuerdoPagoCuentasPendientes();

                    List<BllAcuerdoPagoCuentasPendientes> Rows = new List<BllAcuerdoPagoCuentasPendientes>();

                    Rows = (List<BllAcuerdoPagoCuentasPendientes>)Session["ListAcuerdos"];
                    ...
                    if (Rows.Exists(b => b.Id.ToString() == e.CommandArgument.ToString()))
                    {
                        ...
                        FechaAcuerdo.Text = Row.FechaCreacion.ToString("yyyy-MM-dd");;
                        ModoPago.SelectedValue = Row.DiasPago.ToString();
                        var query= new BllCuotasPagoPendiente();
